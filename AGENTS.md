...
- Do not default to `/tmp` for build contexts, caches, logs, or staging directories.
  - Prefer a repo-local `.codex/`, `.cache/`, `target/`, or another project-scoped scratch path first.
  - Use `/tmp` only for genuinely tiny, disposable scratch files, OS-required temporary paths, or short-lived mount/debug points.
  - If you choose `/tmp`, keep it narrow and explain why that path is specifically appropriate.
